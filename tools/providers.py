...
def evaluate_kubernetes_capacity(
    ...
    need_cpu = float(requirements.get("cpu_cores", 0))
    need_memory = int(requirements.get("memory_bytes", 0))
    need_pods = int(requirements.get("pods", 1))
    ...
    for capacity in capacities:
        ...
        for taint in _untolerated_taints(
            ...
            if taint["value"]:
                description += f"={taint['value']}"

            reasons.append(
                f"Untolerated node taint: "
                f"{description}:{taint['effect']}"
            )

        if headroom["cpu_cores"] < need_cpu:
            reasons.append(
                f"CPU available {headroom['cpu_cores']:.3f} cores "
                f"< required {need_cpu:.3f} cores"
            )

        if headroom["memory_bytes"] < need_memory:
            reasons.append(
                f"Memory available {headroom['memory_bytes']} bytes "
                f"< required {need_memory} bytes"
            )

        if headroom["pods"] < need_pods:
            reasons.append(
                f"Pod slots available {headroom['pods']} "
